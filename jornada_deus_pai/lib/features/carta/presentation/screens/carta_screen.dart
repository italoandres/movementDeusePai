...
///
/// Reconhece automaticamente se o usuário já possui o livro
/// e adapta a experiência de conclusão.
...
class _CartaScreenState extends ConsumerState<CartaScreen>
  ...
  static const _goldSoft = Color(0x59C6A15B);
  ...
  /// Verifica se o usuário já possui acesso ao livro
  Future<void> _checkBookAccess() async {
    ...
    try {
      final profile = await Supabase.instance.client
          .from('profiles')
          .select('has_book_access, access_type')
          .eq('id', user.id)
          .maybeSingle();

      if (mounted && profile != null) {
        setState(() {
          _hasBookAccess = profile['has_book_access'] == true ||
              profile['access_type'] == 'book' ||
  ...
  void _togglePlayPause() {
    if (_videoElement.paused) {
      // FORCE play via the HTML element directly
      _videoElement.play();
  ...
  void _seek(double seconds) {
    final newTime = (_videoElement.currentTime + seconds).clamp(0.0, _videoElement.duration.toDouble());
    _videoElement.currentTime = newTime;
  }
    ...
    return Scaffold(
      ...
      body: Stack(
        ...
        children: [
          ...
          // Overlay transparente que captura taps
          Positioned.fill(
          ...
          // Bottom contemplative area (aparece ao pausar)
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Frase contemplativa (esquerda, abaixo de tudo)
          Align(
            alignment: Alignment.centerLeft,
            child: FadeTransition(
              opacity: _phraseFade,
  ...
  /// Tela de conclusão — adapta para quem já tem o livro vs quem não tem
          ...
          child: FadeTransition(
            ...
            child: Center(
              child: SingleChildScrollView(
                ...
                child: _hasBookAccess
                    ? _buildBookOwnerCompletion(context)
                    : _buildNonOwnerCompletion(context),
              ),
            ),
          ),
        ),
  ...
  /// Para quem JÁ TEM o livro — continuação da caminhada
  Widget _buildBookOwnerCompletion(BuildContext context) {
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        ...
        Text(
          'você pode continuar essa caminhada.',
          style: TextStyle(
        ...
        Text(
          'o livro "não ore, fale com o Pai"\njá está disponível no seu espaço\ndiante do Pai.',
          style: TextStyle(
  ...
  /// Para quem NÃO tem o livro — convite a continuar a jornada
  Widget _buildNonOwnerCompletion(BuildContext context) {
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        const SizedBox(height: 80),
